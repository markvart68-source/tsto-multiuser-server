...
public:
    ...
    bool add_device(const Device&);
    bool device_belongs_to(std::int64_t, const std::string&) const;

    bool create_session(const Session&);
    std::optional<Session> find_session(const std::string&) const;
    ...
    std::map<std::string, std::int64_t> get_currencies(std::int64_t account_id) const;
    bool set_currency(std::int64_t account_id, const std::string& currency, std::int64_t amount);
