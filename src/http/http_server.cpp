...
std::string reason(int status) {
    switch (status) { case 200:return "OK"; case 201:return "Created"; case 400:return "Bad Request"; case 401:return "Unauthorized"; case 404:return "Not Found"; case 405:return "Method Not Allowed"; case 409:return "Conflict"; case 413:return "Payload Too Large"; case 500:return "Internal Server Error"; default:return "Service Unavailable"; }
}
...
void send_all(socket_type socket, const std::string& data) {
    ...
    while (sent < data.size()) {
        const auto count = ::send(socket, data.data() + sent, static_cast<int>(data.size() - sent), 0);
        if (count <= 0) return;
...
void HttpServer::run() {
    ...
    else if (inet_pton(AF_INET, config_.listen_address.c_str(), &address.sin_addr) != 1) { close_socket(server); throw std::runtime_error("listen_address must be an IPv4 address"); }
...
void HttpServer::handle_client(int raw_socket) {
    ...
    const auto header_text = request.substr(0, header_end);
    ...
    while (std::getline(lines, line)) {
        ...
        if (name == "Content-Length" || name == "content-length") { try { content_length = std::stoull(value); } catch (...) { content_length = 0; } }
    }
    if (content_length > config_.max_request_bytes) { send_all(socket, "HTTP/1.1 413 Payload Too Large\r\nConnection: close\r\nContent-Length: 0\r\n\r\n"); return; }
    const std::size_t body_start = header_end + 4;
    while (request.size() < body_start + content_length) { const auto n = ::recv(socket, buffer, sizeof(buffer), 0); if (n <= 0) return; request.append(buffer, static_cast<std::size_t>(n)); }
    http_request.body = request.substr(body_start, content_length);
    const auto query = http_request.path.find('?');
    if (query != std::string::npos) http_request.path.resize(query);
    ...
    if (http_request.path.rfind("/webpanel/", 0) == 0 && http_request.method == "GET") {
        const auto relative = http_request.path.substr(std::string("/webpanel/").size());
        std::filesystem::path file = web_root_ / relative;
        file = file.lexically_normal();
        const auto root = web_root_.lexically_normal();
        const auto file_text = file.generic_string();
        ...
        else { std::ifstream input(file, std::ios::binary); body.assign(std::istreambuf_iterator<char>(input), {}); type = content_type(file); }
    } else if (http_request.path.rfind("/v1/", 0) == 0) {
        const auto response = router_.handle(http_request); status = response.status; body = response.body; type = response.content_type;
